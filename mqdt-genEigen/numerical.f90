...
!     IR
   ...
   subroutine prepare_GEV(nux, T, CTR)
      implicit none
      type(mqdt), intent(inout) :: T
      type(control), intent(in) :: CTR
      real(long), intent(in) :: nux
      call GEV_au(nux, T)
   end subroutine prepare_GEV

   subroutine GEV_au(nux, T)
      implicit none
      type(mqdt), intent(inout) :: T
      real(long),intent(in) :: nux
      integer :: i, j
      real(long), dimension(T%nchan) :: dnu
      dnu(:) = ZERO
      ! be careful about the ordering of the open channels.
      ! We want the open channels to be in the upper block
      do i = 1, T%nchan
         if(T%IP_seq(i) >= T%IPx) then
            dnu(i) = from_nux_get_nu(T, nux, T%IP_seq(i))
            T%dnu(i) = dnu(i) 
           do j = 1, T%nchan
            T%LM(i,j) = T%U(i, j)*sin(PI * (dnu(i) + T%mu(j)))
            T%RM(i,j) = ZERO
           end do 
         else 
           do j = 1, T%nchan
            T%LM(i,j) = T%U(i, j)*sin(PI * T%mu(j))
            T%RM(i,j) = T%U(i, j)*cos(PI * T%mu(j))
           end do 
         endif
      enddo
      return 
   end subroutine GEV_au
...
! iter returned -1 means exceeding the maximum iterations
   ...
   subroutine REALMATINV(N,M,AA,BB,det)
      implicit none
      integer, intent(in) :: N, M
      real(long), PARAMETER :: EPSMACH=2.E-12_long
      real(long), dimension(N, N), intent(inout) :: AA
      real(long), dimension(N, M), intent(inout) :: BB
      real(long), intent(out) :: det
      INTEGER, pointer :: PC(:), PL(:)
      real(long), pointer :: CS(:)
      real(long) :: PV, TT
      real(long) :: PAV
...
!REARRANGEMENT TERMINATED.                                                        
   ...
   function lapack_inv(A) result(Ainv)
    ...
    real(long)            :: Ainv(size(A,1),size(A,2))
    real(long)            :: work(size(A,1))            ! work array for LAPACK
   ...
   end function lapack_inv
  Subroutine lapack_ddgev( H, S, n )

    !Use, Intrinsic :: iso_fortran_env, Only : wp => real64
    use constants, only : wp => long 
    Implicit None
    integer, intent(in) :: n
    Real( wp ), Dimension( 1:N, 1:N ), Intent( In ) :: H
    Real( wp ), Dimension( 1:N, 1:N ), Intent( In ) :: S
