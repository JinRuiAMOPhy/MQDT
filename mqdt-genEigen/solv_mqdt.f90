...
program SOLVMQDT
   ...
   if(T%nop == T%nchan) then
    ...
    if (index(CTR%solver, 'gev') /= 0) then
       call graphic_solv_Greene_GEV(FN, T, S, CTR, G)
    else if(index(CTR%solver, 'tradition') /= 0) then
...
contains
   ...
   subroutine graphic_solv_Greene_GEV(FN, T, S, CTR, G)
      implicit none
      type(fname), intent(in) :: FN
      type(mqdt), intent(inout) :: T
      type(grid), intent(inout) :: G
      ...
      T%tx(2) = T%nux
      print *, 'b4 GEV solver'
      call solve_GEV_dggev(T%nux, T, S, CTR, G, nrt)
      call print_result(T, FN)
      if(CTR%dbg_para_use) then
         write(FN%funit_para_use, fmtstr, advance='no') T%nux
         call print_vector(T%mu, T%nchan, T%nchan, fmtstr, FN%funit_para_use, 'no')
      ...
      if(CTR%yes_dis) close(FN%funit_normdos)
      if(CTR%dbg_para_use) close(FN%funit_para_use)
   end subroutine graphic_solv_Greene_GEV
...
! iter < 0 : exceeding maximum iterations
   ...
   subroutine solve_GEV_dggev(nux, T, S, CTR, G, nrt)
      implicit none
      type(mqdt), intent(inout) :: T
      type(Smat), intent(in) :: S
      type(control), intent(in) :: CTR
...
!     .. Scalar Arguments ..
      ...
      real(long) :: ABNRM, BBNRM
!     .. Array Arguments ..
      LOGICAL, dimension(:), allocatable :: BWORK
      INTEGER, dimension(:), allocatable :: IWORK, indx
      real(long), dimension(:,:), allocatable :: A, B,VR, VL
      real(long), dimension(:), allocatable :: ALPHAI, ALPHAR, beta, lscale, rscale, &
              rconde, rcondv,work, tau

      N = T%nchan
      call interp_Smat(nux, T, S, CTR)
      call prepare_GEV(nux, T, CTR)
      allocate(A(N,N),B(N,N))
      A(1:T%nchan, 1:T%nchan) = T%LM 
      B(1:T%nchan, 1:T%nchan) = T%RM 
      LDVL = N; LDVR = N; LDA = N; LDB = N
      Lwork = max(10*N,2*N*N+8*N+16)
      balanc = 'B'; sense = 'B'; jobvl = 'N'; jobvr ='V'
      !balanc = 'N'
      nrt = T%nop
      allocate(WORK(lwork), vl(N,N), vr(N,N), Bwork(N),Iwork(6+N))
      allocate(alphar(N), alphai(N),beta(N),lscale(N),Rscale(N),&
               rconde(N),rcondv(N),tau(T%nop), indx(T%nop))
      call dggevx( BALANC, JOBVL, JOBVR, SENSE, N, A, LDA, B, LDB, &
                        ALPHAR, ALPHAI, BETA, VL, LDVL, VR, LDVR, ILO, &
                        IHI, LSCALE, RSCALE, ABNRM, BBNRM, RCONDE, &
                        RCONDV, WORK, LWORK, IWORK, BWORK, INFO )
! one might pay attention to the ordering of channels,
! because we want the open channels to be in the upper block.
      do i = 1, T%nop
        if(abs(Beta(T%iopen(i))) < EPS * 1.e2) then
            print *, "**************WARNING Beta(",trim(int2str(T%iopen(i))),&
                     ')=',Beta(T%iopen(i)),'at nux=',nux
        end if 
      end do
      if(INFO /= 0) print *, "**************WARNING INFO != 0", "at ",trim(float2str(nux))," DGGEV fail**************"
      ! Be careful, we have reordered AlphaR(T%iopen(i)! 
      ! need also do this to the vectors. 
      tau(1:T%nop) = (/( modulo(atan(AlphaR(T%iopen(i)) / Beta(T%iopen(i))) / PI, ONE), i = 1, T%nop) /)
      call sort_real_vec(tau, 'decend', T%tau, indx)
      if(CTR%dbg_eqnsolv) then
        
        print * , 'work(1)',work(1)
        print "('Lscale ',1E30.10, ' RSCALE ',1E30.10)", LSCALE, RSCALE
        print "('Anorm ',1E30.20, ' BBNRM ',1E30.20)",ABNRM, BBNRM
        print *, 'AR'
        print "(1E30.20)", ALPHAR(:)
        print *, 'AI'
        print "(1E30.20)", ALPHAI(:)
        print *, 'Beta'
        print "(1E30.20)", BETA(:)
        print *,'rconde'
        print "(1E30.10)", RCONDE
        print *,'rcondV'
        print "(1E30.10)", RCONDV
        print *, 'info',info
        print *,'Miu'
        print "(1E30.20)", T%tau(1:T%nop)
      end if 
      call solu_GEV(nux, T, CTR, G, VR, indx)
      deallocate(WORK,vl, vr, Bwork,Iwork,alphar, alphai,beta,lscale,rscale,rconde,rcondv)
      deallocate(A,B, tau, indx)

      if(CTR%dbg_eqnsolv) then
         write(funit_dbg, "('# eigenchannel mu:')")
         call print_vector(T%mu, T%nchan, print_col_s_fmt, '(f9.4)', funit_dbg)
         do i = 1, T%nop
           call prepare_Fmat(nux, ONE-T%tau(i), T, CTR)
           print "(i3,'th det[F]',1E30.20)",i,T%detF
           print *, "F*A[rho]"
           print "(100E20.10)", matXvec(T%F, T%Ara(:,i))
         end do 
         write(funit_dbg, "('# roots:')")
         call print_vector(T%tau(:), T%nop, print_col_s_fmt, '(f9.4)', funit_dbg)
      end if
   end subroutine solve_GEV_dggev
   

   subroutine solu_GEV(nux, T, CTR, G, VR, indx)
      implicit none
      type(mqdt), intent(inout) :: T
      type(control), intent(in) :: CTR
      type(grid), intent(in) :: G
      ...
      real(long), dimension(:,:), intent(in) :: VR
      integer :: i, i2
! note tau has already been sorted, as stored in indx!
      !print *,  "in solu_GEV T%iopen"
      !print "(100i3)", T%iopen
      do i2 = 1, T%nop
        i = indx(T%iopen(i2))
        if(abs(cos(PI*T%tau(i2))) < EPS*1.e5) &
          print *, 'WARNING very small cos(PI*tau)', cos(PI*T%tau(i2))
        T%Ara(:, i2) = VR(:,i) / cos(PI*T%tau(i2))
        T%Ara(:, i2) = T%Ara(:, i2) / sqrt(inner_prod(T%Ara(:, i2),T%Ara(:, i2)))
        call TIJ(T, CTR, i2)
        !if(CTR%dbg_eqnsolv) then
          !print *,'A('//int2str(i2)//')'
          !print "(1E30.20)", T%Ara(:,i2)
        !end if 
        if(CTR%dbg_solu) &
          print *,'A(',trim(int2str(i2,'(i8)')),')'        
        if(CTR%dbg_solu) &
          call print_vector(T%Ara(:, i2), T%nchan, print_col_s_fmt, '(E13.5)', STDOUT)        
      end do
   end subroutine solu_GEV
