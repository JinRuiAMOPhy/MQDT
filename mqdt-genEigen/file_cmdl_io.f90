...
contains
   ...
   subroutine read_mqdt_input_key(FN, CTR, T, S, G, keys, args)
      ...
      do i = 1, size(keys)
         ...
         else if(keys(i) == 'solver') then
            read(vals(i), *) CTR%solver
   ...
   subroutine initiate(FN, T, S, CTR, G)
      ...
      allocate(T%U(T%nchan, T%nchan), T%Angle(T%n_ang), &
          ...
          T%LM(T%nchan, T%nchan),T%RM(T%nchan, T%nchan),T%dnu(T%nchan))
      ...
      write(funit_dbg, "('# Linear equation solver: ', A)", advance = 'no') &
         CTR%solver
      if(.not.check_item_in_list(keys, 'solver')) then
         write(funit_dbg, "(' (internal)')")
      else
         write(funit_dbg, *)
      end if

      if(CTR%yes_dis .and. index(CTR%solver, 'GEV') /= 0) then
         print *, 'using GEV in the discrete region is not allowd.'
         write(funit_dbg, "('using GEV in the discrete region is not allowd.')")
         stop
      end if 

      write(funit_dbg, "('# secular equation solver: ', A)", advance = 'no') &
         CTR%eqnsolv_method
      if(.not.check_item_in_list(keys, 'eqnsolv_method')) then
         write(funit_dbg, "(' (internal)')")
      else
   ...
   subroutine finalize(T, S, G)
      ...
      if(allocated(T%LM)) deallocate(T%LM)
      if(allocated(T%RM)) deallocate(T%RM)
      if(allocated(T%dnu)) deallocate(T%dnu)
